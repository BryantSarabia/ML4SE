...
\begin{itemize}
    \item \textbf{High ROC-AUC (0.9658)}: The model excels at ranking toxic comments above non-toxic ones
    \item \textbf{Good F1-Score (0.6877)}: Balanced performance between precision and recall
    \item \textbf{High Precision (0.7783)}: When the model flags a comment as toxic, it's correct 78\% of the time
    \item \textbf{Moderate Recall (0.6282)}: The model catches 63\% of actual toxic comments
    \item \textbf{Low Hamming Loss (0.0190)}: Very few incorrect label predictions per sample
    \item \textbf{Low Standard Deviation}: All metrics show $\sigma < 0.025$, indicating robust generalization
\end{itemize}
